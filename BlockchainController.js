...
class BlockchainController {
        ...
        this.app.post("/submitstar", async (req, res) => {
            if (req.body.address && req.body.message && req.body.signature && req.body.star) {
                ...
                try {
                    ...
                    if (block) {
                        console.log('inside block received ' + block);
                        return res.status(200).json(block);
                    } else {
                        return res.status(500).send("An error happened!");
                    }
    ...
    validateChain() {
        this.app.get("/validateChain", async (req, res) => {
            try {
                let validationData = await this.blockchain.validateChain();
                console.log('inside validator ' + validationData)

                if (validationData) {
                    return res.status(200).json(validationData);
                } else {
                    return res.status(404).send("Block validation error!");
                }
            } catch (error) {
                return res.status(500).send("An error occured while calling blockchain function!");
            }
        });
    }
